...
def evaluate(model, device, loss_fn, dataloader, metrics, deltaR, deltaR_dz, model_dir):
    ...
    for key in resolutions_arr:
        ...
        for i in range(1, len(bin_edges)):
            ...
            print("u_perp_i: ", u_perp_i.shape, u_perp_i) #added
            ...
            u_par_scaled_hist.append((np.quantile(u_par_scaled_i,0.84)-np.quantile(u_par_scaled_i,0.16))/2. if len(u_par_scaled_i) > 0 else 0)
